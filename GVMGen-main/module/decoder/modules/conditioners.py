...
class CLIPConditioner(VideoConditioner):
    ...
    def __init__(self, name: str, output_dim: int, device: str, video_len: int = 30, finetune: bool = False, is_qformer: bool = False, qformer: omegaconf.DictConfig = None):
        ...
        if is_qformer:
            ...
            for _, param in self.clip.visual.ln_pre.named_parameters():
                param.requires_grad = False
            for _, param in self.clip.visual.transformer.named_parameters():
                param.requires_grad = False
            self.clip.visual.positional_embedding.requires_grad = False
            self.clip.visual.class_embedding.requires_grad = False
            if not finetune:
                self.clip.visual.proj.requires_grad = False
                for _, param in self.clip.visual.ln_post.named_parameters():
                    param.requires_grad = False
        else:
        ...
        for v in entries:
            if v != "":
                ...
                if self.training:
                    ...
                    video = torch.zeros(videos["video"][0].shape[0], 3, 336, 336).to(self.device)
                videos["video"].append(video)
    ...
    def forward(self, inputs: tp.Dict[str, torch.Tensor]) -> ConditionType:
        ...
        mask = torch.ones(video.shape[0], video.shape[1])
        mask[empty_idx, :] = 0
...
class ChromaStemConditioner(WaveformConditioner):
    ...
    def _load_eval_wavs(self, path: tp.Optional[str], num_samples: int) -> tp.Optional[torch.Tensor]:
        ...
        from decoder.data.audio_dataset import AudioDataset
...
def dropout_condition(sample: ConditioningAttributes, condition_type: str, condition: str) -> ConditioningAttributes:
    ...
    if condition_type not in ['text', 'wav', 'joint_embed', 'video']:
        raise ValueError(
            ...
            f" expected 'text', 'wav', 'joint_embed' or 'video' but got '{condition_type}'"
        )

    if condition not in getattr(sample, condition_type):
        raise ValueError(
            "dropout_condition received an unexpected condition!"
            f" expected wav={sample.wav.keys()} and text={sample.text.keys()}/video={sample.video.keys()}"
...
class ClassifierFreeGuidanceDropout(DropoutModule):
    ...
    def forward(self, samples: tp.List[ConditioningAttributes], type: tp.Optional[str] = 'video') -> tp.List[ConditioningAttributes]:
        """
        Args:
            samples (list[ConditioningAttributes]): List of conditions.
        Returns:
        ...
        samples = deepcopy(samples)
        for condition_type in ["wav", type]:
            for sample in samples:
...
class ConditioningProvider(nn.Module):
    ...
    def tokenize(self, inputs: tp.List[ConditioningAttributes]) -> tp.Dict[str, tp.Any]:
        ...
        assert set(wavs.keys() | joint_embeds.keys()).issubset(set(self.conditioners.keys())), (
            f"Got an unexpected attribute! Expected {self.conditioners.keys()}, ",
            f"got {wavs.keys(), joint_embeds.keys()}"
        )
        if self.cond_type == 'text':
