# This file is based on https://github.com/openai/CLIP/blob/main/clip/model.py
...
class QFormerLayer(nn.Module):
    def __init__(self, d_model, heads, layer_idx: int, drop_out, layer_norm_eps, cross_attention_frequency):
        ...
        self.intermediate_query = QFormerIntermediate(d_model, d_model * 2)
        self.output_query = QFormerOutput(d_model * 2, d_model, layer_norm_eps)
...
class QFormerEncoder(nn.Module):
    def __init__(self, d_model, heads, num_hidden_layers, dropout_prob, layer_norm_eps, cross_attention_frequency):
        super().__init__()
        self.num_hidden_layers = num_hidden_layers
        self.layer = nn.ModuleList(
            [QFormerLayer(d_model, heads, layer_idx, dropout_prob, layer_norm_eps, cross_attention_frequency) for layer_idx in range(num_hidden_layers)]
        )

    def forward(self, hidden_states: torch.Tensor, x: torch.Tensor):
        for i in range(self.num_hidden_layers):
            layer_module = self.layer[i]
            hidden_states = layer_module(hidden_states, x)
        return hidden_states

class QFormerModel(nn.Module):
    def __init__(self, d_model, heads, layer_norm_eps, hidden_dropout_prob, qformer_layers, cross_attention_frequency):
        super().__init__()
        self.layernorm = nn.LayerNorm(d_model, eps=layer_norm_eps)
        self.dropout = nn.Dropout(hidden_dropout_prob)

        self.encoder = QFormerEncoder(d_model, heads, qformer_layers, hidden_dropout_prob, layer_norm_eps, cross_attention_frequency)

    def forward(self, query_token: torch.Tensor, x: torch.Tensor):
        hidden_states = self.layernorm(query_token)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.encoder(hidden_states, x)
        return hidden_states

class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, 
                 is_qformer: bool = False, query_num: int = 16, query_type: str = "mean",
                 layer_norm_eps: float = 1e-12, hidden_dropout_prob: float = 0.1, qformer_layers: int = 6, cross_attention_frequency: int = 2):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        ...
        self.is_qformer = is_qformer
        self.query_num = query_num # 1, 2, 4, 8, 16, 32
        self.query_type = query_type # mean, sum
        if is_qformer:
    ...
    def initialize_parameters(self, width, layers, cross_attention_frequency):
        proj_std = (width ** -0.5) * ((2 * layers) ** -0.5)
        attn_std = width ** -0.5
        fc_std = (2 * width) ** -0.5
        for block in self.qformer.encoder.layer:
            ...
            if block.layer_idx % cross_attention_frequency == 0:
                nn.init.normal_(block.crossattention.in_proj_weight, std=attn_std)
                nn.init.normal_(block.crossattention.out_proj.weight, std=proj_std)
                nn.init.normal_(block.cross_output.dense.weight, std=fc_std)

    def forward(self, x: torch.Tensor,rhythm: torch.Tensor = None):
...
class CLIP(nn.Module):
    def __init__(self,
                 ...
                 is_qformer: bool,
                 qformer_cfg: omegaconf.DictConfig
                 ):
        super().__init__()
...
def build_model(state_dict: dict, is_qformer: bool, qformer_cfg: omegaconf.DictConfig):
